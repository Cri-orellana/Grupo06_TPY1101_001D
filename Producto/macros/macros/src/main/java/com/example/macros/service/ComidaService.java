package com.example.macros.service;
...
import com.example.macros.model.ComidaDto;
...
public class ComidaService {
    ...
    public ComidaDto getComidaByBarra(String barra) {
        ...
        try {
            // Creamos las cabeceras obligatorias para identificar tu app ante Open Food Facts
            HttpHeaders headers = new HttpHeaders();
            headers.set("User-Agent", "MiAppNutricion/1.0 (Android; tu-correo@dominio.com)");
            HttpEntity<String> entity = new HttpEntity<>(headers);

            // Cambiamos getForObject por exchange para poder enviarle las cabeceras (headers)
            ...
            if (comidaData == null) return null;

            // Mantiene tu lógica exacta de conversión a tu DTO
    ...
    public List<ComidaDto> getComidasByNombre(String nombre) {
        String url = String.format("%s/cgi/search.pl?search_terms=%s&search_simple=1&action=process&json=1", BASE_URL, nombre);
        try {
